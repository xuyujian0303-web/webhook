...
本机新增抓包 `D:\文档\桌面\ems filter.pcapng` 与导出表 `ems filter.xlsx` 已用于核对筛选请求。请求方法为 `querySaleDetailList`，descriptor 为 `0x172f`，容器包含 15 个参数。固定参数 `0x2712=20230923`；日期字段 `3/4` 分别是开始和结束日期。

| EMS 参数 | 筛选字段 | 抓包映射 |
|---|---|---|
| 5 | 销售机构 | SQL 风格 `in ('G899','G889')` |
| 6 | 销售类型 | `0=销售, 1=退货, 2=换货, 3=预购` |
| 9/10 | 整单金额 | 下限/上限 |
| 22/23 | 单价 | 下限/上限 |
| 24/25 | 折扣 | 下限/上限 |
| 20 | 季号 | 例如 `26FW` |
...
真实只读查询使用机构 `G899/G889`、日期 `2026-09-22` 至 `2026-09-23`、金额 `10000-20000`、单价 `0-50000`、折扣 `0-100`、季号 `26FW`、出货组别 `26FWG6`、款色码 `JACH619CNY0` 及类型 `0/1/2/3`，返回 1 单；该单为 `G899` 的预购单，日期为 `2026-09-22`，款色码为 `JACH619CNY0`。查询只读，未发送 Webhook。

保持其余条件完全不变的销售类型组合复测结果：

| EMS 销售类型 | 原始回包订单数 | 回包类型分布 |
|---|---:|---|
| `0/1/2` | 0 | 空结果（有效响应，非连接错误） |
| `1/3` | 1 | 预购 1 单（编码 3） |

款色码字段 32 仍不能视为服务器端可靠过滤：分别提交 `JACH616CNY0` 和 `JACH619CNY0` 的对照查询都返回相同的 `JACH619CNY0` 单据。因此 GUI 将其标示为“EMS查询参数/本地校验”，本地精确规则必须保留。过滤查询发生连接错误时不得退回无筛选的宽查询。
